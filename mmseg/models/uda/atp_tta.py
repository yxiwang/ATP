...
class SePiCo(UDADecorator):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img, target_img_metas, target_gt_semantic_seg):
        ...
        # update distribution
        ema_target_feat = self.get_ema_model().extract_auxiliary_feat(weak_target_img)
        mean = {}
        covariance = {}
        bank = {}
        if self.contrast_mode == 'multiple_select':
            for idx in range(len(self.calc_layers)):
                feat, mask = contrast_preparations(ema_target_feat[idx], pseudo_label.unsqueeze(1), self.enable_avg_pool,
            ...
            #print("pseudo label shape", pseudo_label.shape)
            feat, mask = contrast_preparations(ema_target_feat, pseudo_label.unsqueeze(1), self.enable_avg_pool,
                                               self.scale_min_ratio, self.num_classes, self.ignore_index)
        ...
        source_losses = self.get_model().forward_train(img, img_metas, gt_semantic_seg, return_feat=False,
                                                       mean=mean, covariance=covariance, bank=bank, mode='dec')
        ...
        #"""
        if self.local_iter > -1: #< self.start_distribution_iter:
            # target entropy cl
            target_ent_loss = self.get_model().forward_train(target_img, target_img_metas, pseudo_label.unsqueeze(1), mode='dec')
            target_loss, target_log_vars = self._parse_losses(target_ent_loss)
